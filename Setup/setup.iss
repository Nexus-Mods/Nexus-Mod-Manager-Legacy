...
[Setup]
AppName={#MyAppLinkName}
AppID=6af12c54-643b-4752-87d0-8335503010de
AppVersion={#MyAppVersion}
AppVerName={#MyAppLinkName} {#MyAppVersion}
...
DefaultGroupName={#MyAppLinkName}
DefaultDirName={pf}\{#MyAppLinkName}
UninstallDisplayName={#MyAppLinkName}
...
Name: associateNxmFiles; Description: &Associate *.NXM files with {#MyAppLinkName}; GroupDescription: Other tasks:;
Name: associateNxmUrls; Description: Associate NXM: &URLs with {#MyAppLinkName}; GroupDescription: Other tasks:;
Name: associateFomodFiles; Description: &Associate *.FOMOD files with {#MyAppLinkName}; GroupDescription: Other tasks:;
Name: associateOmodFiles; Description: &Associate *.OMOD files with {#MyAppLinkName}; GroupDescription: Other tasks:;
...
[Icons]
Name: {group}\{#MyAppLinkName}; Filename: {app}\{#MyExeName}; WorkingDir: {app}
Name: {group}\{#MyAppLinkName} (Trace Mode); Filename: {app}\{#MyExeName}; Parameters: -trace; WorkingDir: {app}
Name: {group}\{cm:UninstallProgram,{#MyAppLinkName}}; Filename: {uninstallexe}; WorkingDir: {app}
Name: {commondesktop}\{#MyAppLinkName}; Filename: {app}\{#MyExeName}; Tasks: desktopicon; WorkingDir: {app}
Name: {userappdata}\Microsoft\Internet Explorer\Quick Launch\{#MyAppLinkName}; Filename: {app}\{#MyExeName}; Tasks: quicklaunchicon; WorkingDir: {app}

[Run]
Filename: {app}\{#MyExeName}; Description: {cm:LaunchProgram,{#MyAppLinkName}}; Flags: nowait postinstall skipifsilent

[Registry]
;.nxm
Root: HKCR; Subkey: .nxm; ValueType: string; ValueName: ; ValueData: NXM_File_Type; Flags: uninsdeletekey; Tasks: associateNxmFiles
Root: HKCR; Subkey: NXM_File_Type; ValueType: string; ValueName: ; ValueData: {#MyAppLinkName} Mod Archive; Flags: uninsdeletekey; Tasks: associateNxmFiles
  ...
  case CurUninstallStep of
    usUninstall:
      begin
        ...
        mRes := MsgBox('Do you want to remove all of {#MyAppLinkName}''s configuration files? Doing so will reset all of {#MyAppLinkName}''s settings. Either option will keep your mods intact.', mbConfirmation, MB_YESNO or MB_DEFBUTTON2)
        if mRes = IDYES then
          begin
            mPub := '{#MyPublisher}';
